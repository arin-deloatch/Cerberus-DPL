...
def _median(values: List[Union[int, float]]) -> float:
    """
    ...
    return float(sorted_values[middle_index]) if count % 2 else (sorted_values[middle_index - 1] + sorted_values[middle_index]) / 2.0
...
def _iqr(values: List[Union[int, float]]) -> float:
    """
...
def _mad(values: List[Union[int, float]]) -> float:
    """
...
def summarize_by_topic(document_records: List[DocRecord]) -> List[TopicStats]:
    ...
    for document_record in document_records:
        topic_to_token_lengths.setdefault(document_record.topic, []).append(document_record.token_len)

    topic_statistics: List[TopicStats] = []
    for topic_id, token_lengths in sorted(topic_to_token_lengths.items(), key=lambda item: item[0]):
        document_count = len(token_lengths)
        mean_length = sum(token_lengths) / document_count if document_count else 0.0
        variance = (sum((length - mean_length) ** 2 for length in token_lengths) / (document_count - 1)) if document_count > 1 else 0.0
        topic_statistics.append(TopicStats(
            topic=topic_id,
            n_docs=document_count,
            mean_len=mean_length,
            median_len=_median(token_lengths),
            min_len=min(token_lengths) if document_count else 0.0,
            max_len=max(token_lengths) if document_count else 0,
